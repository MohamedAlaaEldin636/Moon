...
class AdsUseCase @Inject constructor(
  ...
  fun getFilterDetails2(categoryId: Int, subCategoryId: Int, showProgress: Boolean = true): Flow<Resource<BaseResponse<ResponseFilterDetails?>>> = flow {
	...
	suspend fun addAdvertisement(
		...
		longitude,
		address,
		if (isStore) null else city_id,
		price,
		if (is_negotiable) 1 else 0,
		brand_id,
		description,
		propertiesIds,
		if (isStore) price_before else null,
		if (isStore) store_category_id else null,
		if (isStore) store_sub_category_id else null,
	)
