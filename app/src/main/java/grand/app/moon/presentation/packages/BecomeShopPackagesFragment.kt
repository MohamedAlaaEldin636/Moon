...
class BecomeShopPackagesFragment : BaseFragment<FragmentBecomeShopPackagesBinding>() {
	...
	private val viewModel by viewModels<BecomeShopPackagesViewModel>()
	...
	private inner class PagerAdapter : FragmentStateAdapter(this) {
		override fun getItemCount(): Int = viewModel.allPackages.size

		override fun createFragment(position: Int): Fragment = PageBecomeShopPackageFragment().also {
			MyLogger.e("euiwhid -> ch 0 -> ${viewModel.allPackages.getOrNull(position).toJsonInlinedOrNull()}")

			it.arguments = bundleOf(
				PageBecomeShopPackageFragment.ARGS_ITEM to viewModel.allPackages.getOrNull(position).toJsonInlinedOrNull()
			)
		}
	}
